...
if ($LogPath -and (Test-Path $LogPath)) {
    Start-Transcript -Path "$LogPath/WindowsHealingSystem.log" -Append -IncludeInvocationHeader -Force | Out-Null
}
else {
    Start-Transcript -Path "$PSScriptRoot/WindowsHealingSystem.log" -Append -IncludeInvocationHeader -Force | Out-Null
}
...
function ShowAppSelectionForm {
    ...
    $form.Text = "WindowsHealingSystem Application Selection"
...
function PrintHeader {
    ...
    $fullTitle = " WindowsHealingSystem Script - $title"
...
function CreateSystemRestorePoint {
    ...
    $createRestorePointJob = Start-Job { 
        ...
        if ($recentRestorePoints.Count -eq 0) {
            try {
                Checkpoint-Computer -Description "Restore point created by WindowsHealingSystem" -RestorePointType "MODIFY_SETTINGS"
...
if ((Get-AppxPackage -Name "*Microsoft.DesktopAppInstaller*") -and ([int](((winget -v) -replace 'v','').split('.')[0..1] -join '') -gt 14)) {
    ...
    if (-not $Silent) {
        Write-Warning "Winget is not installed or outdated. This may prevent WindowsHealingSystem from removing certain apps."
        Write-Output ""
        Write-Output "Press any key to continue anyway..."
        $null = [System.Console]::ReadKey()
    }
...
# Hide progress bars for app removal, as they block WindowsHealingSystem's output
...
if ($script:Params.ContainsKey("Sysprep")) {
    ...
    if ($WinVersion -lt 22000) {
        Write-Host "Error: WindowsHealingSystem Sysprep mode is not supported on Windows 10" -ForegroundColor Red
        AwaitKeyToExit
    }
}
...
if ((-not $script:Params.Count) -or $RunDefaults -or $RunWin11Defaults -or $RunSavedSettings -or ($SPParamCount -eq $script:Params.Count)) {
    ...
    switch ($Mode) {
        ...
        '4' {
            PrintHeader 'Custom Mode'
            Write-Output "WindowsHealingSystem will make the following changes:"
