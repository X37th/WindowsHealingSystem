...
function LoadAppsDetailsFromJson {
    ...
    # --- PERFORMANCE: Cache all installed Appx packages in one call ---
    $installedAppxSet = @{}
    try {
        Get-AppxPackage -AllUsers | ForEach-Object {
            $installedAppxSet[$_.Name] = $true
        }
    } catch {
        # If Get-AppxPackage fails entirely, fall back to showing all apps
        $installedAppxSet = $null
    }
    ...
    foreach ($appData in $jsonContent.Apps) {
        ...
        if ($OnlyInstalled) {
            ...
            foreach ($appId in $appIdArray) {
                if (($InstalledList -like "*$appId*") -or ($null -eq $installedAppxSet) -or ($installedAppxSet.ContainsKey($appId))) {
                    $isInstalled = $true
                    break
                }
