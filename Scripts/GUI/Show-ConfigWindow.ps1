...
function Get-SelectedApplications {
    ...
    foreach ($expander in $AppsPanel.Children) {
        if ($expander -is [System.Windows.Controls.Expander] -and $expander.Content -is [System.Windows.Controls.WrapPanel]) {
            foreach ($child in $expander.Content.Children) {
        ...
        elseif ($expander -is [System.Windows.Controls.CheckBox] -and $expander.IsChecked) {
            $selectedApps += $expander.Tag
        }
    }
...
function Get-DeploymentSettings {
    ...
    $appRemovalScopeAllUsers = $Owner.FindName('AppRemovalScopeAllUsers')
    if ($appRemovalScopeAllUsers) {
        # Determine index via sibling chip state
        $allUsers    = $Owner.FindName('AppRemovalScopeAllUsers')
        $currentUser = $Owner.FindName('AppRemovalScopeCurrentUser')
        $targetUser  = $Owner.FindName('AppRemovalScopeTargetUser')
        $scopeIdx = 0
        if ($currentUser -and $currentUser.IsChecked) { $scopeIdx = 1 }
        elseif ($targetUser -and $targetUser.IsChecked) { $scopeIdx = 2 }
        $deploySettings += @{ Name = 'AppRemovalScopeIndex'; Value = $scopeIdx }
    }
...
function Apply-ImportedApplications {
    ...
    foreach ($expander in $AppsPanel.Children) {
        ...
        # Legacy support
        elseif ($expander -is [System.Windows.Controls.CheckBox]) {
            $expander.IsChecked = ($AppIds -contains $expander.Tag)
        }
    }
}
...
function Apply-ImportedDeploymentSettings {
    ...
    $currentUser = $Owner.FindName('AppRemovalScopeCurrentUser')
    $targetUser  = $Owner.FindName('AppRemovalScopeTargetUser')
    if ($lookup.ContainsKey('AppRemovalScopeIndex') -and $allUsers) {
        switch ([int]$lookup['AppRemovalScopeIndex']) {
            0 { if ($allUsers)    { $allUsers.IsChecked    = $true } }
            1 { if ($currentUser) { $currentUser.IsChecked = $true } }
            2 { if ($targetUser)  { $targetUser.IsChecked  = $true } }
        }
    }
...
function Export-Configuration {
    ...
    $saveDialog.FileName = "WindowsHealingSystem-Config-$(Get-Date -Format 'yyyyMMdd').json"
